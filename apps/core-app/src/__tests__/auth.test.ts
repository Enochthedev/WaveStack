...
import { getKeypair, resetKeypair } from "../modules/auth/keys";
...
describe("Auth Module - Key Management", () => {
  beforeEach(() => {
    // Clear the module-level cache so each test gets a fresh keypair
    resetKeypair();
...
describe("Auth Module - JWT Claims", () => {
  ...
  it("should handle multiple scope formats", async () => {
    ...
    const token1 = jwt.sign({ sub: "user1", scope: "read write delete" }, pair.priv, {
      algorithm: "RS256",
      keyid: pair.kid,
    });
    ...
    // Array scopes
    const token2 = jwt.sign({ sub: "user2", scopes: ["read", "write", "delete"] }, pair.priv, {
      algorithm: "RS256",
      keyid: pair.kid,
    });
...
describe("Auth Module - Token Expiration", () => {
  it("should reject expired tokens", async () => {
    ...
    const token = jwt.sign({ sub: "test-user" }, pair.priv, {
      algorithm: "RS256",
      keyid: pair.kid,
