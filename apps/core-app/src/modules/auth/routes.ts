...
export default async function authRoutes(app: FastifyInstance) {
    ...
    try {
      if (mode === "hs256") {
        ...
        const verify = promisify(
          (t: string, s: jwt.Secret, o: jwt.VerifyOptions, cb: jwt.VerifyCallback) =>
            jwt.verify(t, s, o, cb),
        );
  ...
  app.post("/token", async (req, reply) => {
    ...
    const client = clients.find(
      (c: any) => c.client_id === client_id && c.client_secret === client_secret,
    );
    if (!client) return reply.code(401).send({ error: "invalid_client" });

    // normalize incoming/allowed scopes
    const requestedScopes: string[] = Array.isArray(scope)
      ? scope
      : typeof scope === "string"
        ? scope.trim().split(/\s+/).filter(Boolean)
        : [];

    const allowedScopes: string[] = Array.isArray(client.scopes)
      ? client.scopes
      : typeof client.scopes === "string"
        ? client.scopes.trim().split(/\s+/).filter(Boolean)
        : [];
...
function extractScopes(decoded: any): string | undefined {
  const raw =
    decoded?.scopes || decoded?.scope || decoded?.scp || decoded?.permissions || decoded?.roles;
  if (!raw) return;
