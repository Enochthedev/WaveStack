"""
WaveStack AI Core
Unified AI service: personality engine, knowledge/RAG, model routing, agent orchestration
"""
import asyncio
import logging
...
# Create FastAPI app
app = FastAPI(
    title="WaveStack AI Core",
    description="Unified AI: personality, knowledge/RAG, model routing, agent orchestration",
    version="1.0.0",
    lifespan=lifespan
)
...
# Personality routers (original ai-personality)
app.include_router(personality.router, prefix="/api/v1/personality", tags=["Personality"])
app.include_router(content.router, prefix="/api/v1/content", tags=["Content"])
app.include_router(memory.router, prefix="/api/v1/memory", tags=["Memory"])
app.include_router(learning.router, prefix="/api/v1/learning", tags=["Learning"])

# Knowledge/RAG routers — mounted at same paths as standalone service for compatibility
# These will be wired up when the sub-modules are fully integrated
...
async def health_check():
    ...
    return {
        ...
        "modules": ["personality", "knowledge", "model-router", "orchestrator"],
    }
